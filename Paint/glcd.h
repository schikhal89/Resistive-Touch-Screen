...
{
		uchar endx,endy;
		endx = startx + length *  cos(TORADIAN(angle));
		endy = starty + length *  sin(TORADIAN(angle));
...
program memory i.e FLASH and note SRAM
...
/*
	This is  a function to display a small image on lcd
*/
void GlcdPutImage_P(const uchar *img,uchar x,uchar y,uchar bytesinline,uchar noofvertlines,uchar changetype)
{
	uchar lines,bytes,bit,temp;
	for(bytes=0;bytes<bytesinline;bytes++)
		for(lines=0;lines<noofvertlines;lines++)
			{
			temp = pgm_read_byte(&img[bytes*noofvertlines+lines]);
			for(bit=0;bit<8;bit++)
				{
					if(temp & 0x01) 
					{
...
void GlcdShowChar_P(uchar ch,uchar x,uchar y,uchar FontWidth,uchar FontHeight,uchar changetype)
{
	...
	GlcdPutImage_P(&font5X8[(ch - ' ')*FontWidth] , x, y, FontHeight/8, FontWidth, changetype);
}
...
void GlcdShowString_P(uchar *str,uchar x,uchar y,uchar FontWidth,uchar FontHeight,uchar changetype)
{
	...
	{
		if(FontWidth == 5 && FontHeight == 8)
			GlcdPutImage_P(&font5X8[(str[temp] - ' ')*(FontWidth+1)+1] , x, y, FontHeight/8, FontWidth, changetype);
			x +=  font5X8[(str[temp] - ' ')*(FontWidth+1)] +1;
	}		
}
